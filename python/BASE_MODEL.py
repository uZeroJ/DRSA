...
class SparseData():
    ...
    def shuffle(self):
        ...
        return self.data[self.index], self.seqlen[self.index], self.labels[self.index], \
               self.market_price[self.index]
        ...
        for line in fi:
            ...
            if all:
                if bid_price <= market_price:
                    self.data.append(
                        t_indices)  # data only conclude indices use this to get embedding
                ...
                if bid_price <= market_price:
                    if not win:
                        ...
                        self.market_price.append(market_price / discount)
                        self.labels.append(
                            [1., 0.])  # so far we always lose, it means we still survial
                else:
                    if win:
                        # bid_price = 3
                        # market_price = 2
    ...
    def next(self, batch_size):
        ...
        return np.array(batch_data), np.array(batch_labels), np.array(batch_seqlen), np.array(
            batch_market_price)
...
class BASE_RNN():
    ...
    def get_survival_data(self, model, sess):
        ...
        while alltestdata.finish_epoch == False:
            test_batch_x, test_batch_y, test_batch_len, test_batch_market_price = alltestdata.next(
                self.BATCH_SIZE)
    ...
    def create_graph(self):
        ...
        if self.DNN_MODEL:
            ...
            preds = tf.reshape(tf.stack(outlist, axis=0), [self.BATCH_SIZE, self.MAX_SEQ_LEN],
                               name="preds")
        else:
        ...
        map_parameter = tf.concat([batch_rnn_survival_rate,
                                   tf.cast(tf.reshape(self.tf_bid_len, [BATCH_SIZE, 1]),
                                           tf.float32)],
                                  1)
        ...
        log_minus = tf.log(tf.add(tf.transpose(rate_result)[2] - tf.transpose(rate_result)[1],
                                  1e-20))  # todo debug
        ...
        self.predict = tf.transpose(tf.stack([self.final_survival_rate, final_dead_rate]),
                                    name="predict")
        ...
        self.anlp_train_op = optimizer_anlp.apply_gradients(list(zip(anlp_grads, tvars)),
                                                            name="anlp_train_op")
    ...
    def train_test(self, sess):
        ...
        for step in range(1, self.TRAING_STEPS + 1):
            ...
            batch_x, batch_y, batch_len, batch_market_price, win = self.train_data.next(
                self.BATCH_SIZE)
            if self.ONLY_TRAIN_ANLP:
                ...
                if win:  # if win
                    ...
                    _, train_loss, train_outputs = sess.run(
                        [self.train_op, self.cost, self.predict], feed_dict={self.tf_x: batch_x,
                                                                             self.tf_y: batch_y,
                                                                             self.tf_bid_len: batch_len,
                                                                             self.tf_market_price: batch_market_price,
                                                                             self.tf_control_parameter: [
    ...
    def save_model(self):
        ...
        self.saver.save(self.sess, "./saved_model/model" + self.filename,
